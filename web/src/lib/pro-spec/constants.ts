 ...
 */
export const API_FORMATS: ApiFormat[] = ['mj-proxy', 'gemini', 'dalle', 'openai-chat', 'claude', 'koukoutu', 'video-unified', 'openai-video', 'grok-video-chat']
...
export const MODEL_API_FORMAT_OPTIONS: Record<ModelType, ApiFormat[]> = {
  // 绘图模型
  // 通用约定：图像统一走 /v1/images/generations（dalle 兼容协议），
  // 仅在用户明确希望走聊天形式接收图像时才用 openai-chat。
  'midjourney': ['mj-proxy'],
  'gemini': ['dalle', 'gemini', 'openai-chat'],
  'flux': ['dalle'],
  'dalle': ['dalle'],
  'doubao': ['dalle'],
  'gpt4o-image': ['openai-chat'],  // gpt-4o 走 chat completions 输出图
  'gpt-image': ['dalle', 'openai-chat'],
  'sora-image': ['dalle', 'openai-chat'],
  'grok-image': ['dalle', 'openai-chat'],
  'qwen-image': ['dalle', 'openai-chat'],
  'z-image': ['dalle'],
  'agnes-image': ['dalle'],
  'riverflow': ['openai-chat'],
  'koukoutu': ['koukoutu'],
  ...
  // 视频模型
  // 注: sora/veo/kling/seedance 走 OpenAI Video 协议(/v1/videos); video-unified 为旧聚合端点(/v1/video/create)
  'jimeng-video': ['video-unified'],
  'veo': ['openai-video', 'video-unified'],
  'sora': ['openai-video', 'video-unified'],
  'kling': ['openai-video', 'video-unified'],
  'grok-video': ['openai-video', 'grok-video-chat', 'video-unified'],
  'agnes-video': ['openai-video'],
}
...
export const DEFAULT_MODEL_NAMES: Record<ModelType, string> = {
  ...
  'riverflow': 'sourceful/riverflow-v2.5-pro:free',
...
export const API_FORMAT_LABELS: Record<ApiFormat, string> = {
  ...
  'grok-video-chat': 'Grok Video (Chat)',
}
...
export const MODEL_TYPE_ICONS: Record<ImageModelType, string> = {
  ...
  'agnes-image': 'i-heroicons-photo',
  'riverflow': 'i-heroicons-sparkles',
...
export const TASK_CARD_MODEL_DISPLAY: Record<ImageModelType | VideoModelType, { label: string; color: string }> = {
  ...
  'agnes-image': { label: 'Agnes', color: 'bg-fuchsia-500/80' },
  'riverflow': { label: 'Riverflow', color: 'bg-sky-500/80' },
  ...
  'kling': { label: 'Kling', color: 'bg-pink-500/80' },
  'grok-video': { label: 'Grok', color: 'bg-red-500/80' },
  'agnes-video': { label: 'Agnes', color: 'bg-fuchsia-500/80' },
}

// 旧的 MODELS_WITH_* / MODELS_WITHOUT_REFERENCE_IMAGE 列表已被
// app/shared/registry.ts 的 getModelCapabilities() 取代——单一真相、按模型族细粒度配置。
// 历史代码若需判断"X 模型是否支持 Y 参数"，请改用：
//   import { getModelCapabilities } from '~/shared/registry'
//   getModelCapabilities(modelType).size  // 是否支持 size 参数
 ...
 * 默认对话压缩 Prompt
 */
export const DEFAULT_COMPRESS_PROMPT = `将以上对话压缩为简洁的上下文摘要，以便继续后续对话。

**压缩要求**：
- 摘要长度：根据原文长度动态调整（原文 5KB → ~1000字，原文 20KB → ~2000字，原文 40KB → ~4000字）
- 保留关键信息：讨论的核心话题、重要结论、技术细节、代码片段、用户需求、待解决问题
- 忽略冗余内容：重复讨论、无关闲聊、已解决的临时问题
- 使用第三人称客观叙述，不要包含"本次对话"、"用户和助手"等元描述
- 直接陈述事实和结论，不要总结性开头或结尾
- 仅总结对话内容，而不要包含系统提示词中的内容
...
export const USER_SETTING_DEFAULTS: Record<UserSettingKey, UserSettingValue> = {
  ...
  // 开发者模式默认值
  [USER_SETTING_KEYS.DEVELOPER_MODE]: false,
}
...
/**
 * 规范化上游 baseUrl：去除首尾空白与末尾斜杠
 * - 使用场景：
 *   - providerConnection.ts: 运行时拼接 URL 前规范化（兼容历史脏数据）
 *   - upstreams 创建/更新 API: 存储前清洗
 *   - QuickConnect 向导: 提交前规范化
 * 防止 `https://x.com/` + `/v1/...` 拼成 `//v1` 双斜杠导致请求失败。
 */
...
export const DEVELOPER_MODE_ENABLED = 'developer_mode_enabled'
